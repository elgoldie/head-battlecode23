...
public class LauncherAI extends RobotAI {
    ...
    public MapLocation closestDistressedHeadquarters() throws GameActionException {
        MapLocation loc = null;
        int dist = Integer.MAX_VALUE;
        for (int i = 0; i < 4; i++) {
            MapLocation hqLoc = comm.readLocation(i);
    ...
    public void run() throws GameActionException {
        ...
        MapLocation distressedHQ = closestDistressedHeadquarters();
        if (distressedHQ != null) {
            tryMoveOrWander(pathing.findPath(distressedHQ));
            return;
        } else {
